...
class MyClass {
    ...
    processPrintStatement(text) {
        ...
        if (text.includes('floppy disk mounted')) {
            setTimeout( () => {
                if (myClass.rivetsData.initialInstallation) {
                    myClass.sendDosCommands('imgmake \"' + this.base_name + ".img\" -t " + this.initialHardDrive + "\n" + 'imgmount c \"' + this.base_name + ".img\na:\n");
        ...
        if (text.includes('Parsing command line')) {
            myClass.parsingCommands = text + myClass.parsingCommands;
            if (myClass.parsingCommands.length > 500)
                myClass.parsingCommands = myClass.parsingCommands.substr(0, 480);
        }
        ...
        if (text.includes('x ==')) {
            if (text.includes('x == 2')) {
                ...
                if (text.includes('x == 0')) {//this means we explicitly selected shutdown so go to DOS
                } else {
        ...
        if (text.includes('DEBUG_ShowMsg: pixratio 1.000') && myClass.loadSavestateAfterBoot) {
    ...
    playGame(name) {
        ...
        fetch('./' + name).then(response => {
            ...
            const contentLength = 94316544; //response.headers.get('Content-Length');
            ...
            reader.read().then(function process({done, value}) {
                ...
                const receivedLength_ = Math.ceil(receivedLength / 1000000);
                const contentLength_ = Math.ceil(contentLength / 1000000);
    ...
    async parseMultipleFiles() {
        ...
        for (let i = 0; i < this.multiFiles.length; i++) {
            ...
            } else if (file.name.toLocaleLowerCase().endsWith('iso') || file.name.toLocaleLowerCase().endsWith('.cue')) {
    ...
    async LoadEmulator(byteArray) {
        ...
        if (this.rivetsData.initialInstallation) {
            ...
            } else if (this.rom_name.toLocaleLowerCase().endsWith('.zip') || this.rom_name.toLocaleLowerCase().endsWith('.7z')) {
        ...
        this.sendDosControls = Module.cwrap('neil_send_dos_controls', null, ['string', 'string', 'string', 'array', 'number', 'string', 'string']);
        //arrays are always unsigned byte arrays
        this.enableSaving = Module.cwrap('neil_enabled_saving', null, ['number']);

        Module.callMain();

        if (navigator.userAgent.toLocaleLowerCase().includes('windows') || navigator.userAgent.toLocaleLowerCase().includes('macintosh')) {
            this.enableSaving(9);
        } else {
            this.enableSaving(12);
        }
    ...
    async loadRom(noIso) {
        ...
        if (noIso) {
            ...
            if (startupScript)
                this.configuration.startupScript = startupScript;
            if (cpu)
                this.rivetsData.cpu = cpu;
            if (ram)
                this.ram = ram;
            if (harddrive)
                this.initialHardDrive = harddrive;
    ...
    async initAudio() {
        ...
        setInterval( () => {
            ...
            this.pcmPlayer.port.postMessage({
                type: 'update-buffer',
                buffer: Array.from(this.audioBufferResampled),
    ...
    saveDrive() {
        let bytes = Module.FS.readFile('/' + this.base_name + '.img');
    ...
    SaveStateEvent() {
        ...
        let compressed = Module.FS.readFile('/save/1.sav');
        //this is a Uint8Array
        ...
        xhr.open("POST", this.rivetsData.settings.CLOUDSAVEURL + "/SendStaveState?name=" + this.base_name + '.savestate.doswasmx' + "&password=" + this.rivetsData.password + "&emulator=doswasmx", true);
        xhr.send(compressed);
    ...
    async loadHardDriveDiffs(byteArray) {
        ...
        let promise = new Promise(function(resolve, reject) {
            ...
            oReq.open("GET", myClass.rivetsData.settings.CLOUDSAVEURL + "/LoadStaveState?name=" + myClass.base_name + '.doswasmx' + "&password=" + myClass.rivetsData.password, true);
    ...
    async applyHardDriveDiffs(byteArrayDiffs, resolve) {
        ...
        while (pointer < byteArrayDiffs.length) {
            let index = byteArrayDiffs[pointer] + (byteArrayDiffs[pointer + 1] * 256) + (byteArrayDiffs[pointer + 2] * 256 * 256) + (byteArrayDiffs[pointer + 3] * 256 * 256 * 256);
            pointer += 4;

            let length = byteArrayDiffs[pointer] + (byteArrayDiffs[pointer + 1] * 256) + (byteArrayDiffs[pointer + 2] * 256 * 256) + (byteArrayDiffs[pointer + 3] * 256 * 256 * 256);
            pointer += 4;
    ...
    async saveHardDriveDiffs() {
        ...
        compareHardDrive = Module.FS.readFile('/' + this.base_name + '.img');
        //this is a Uint8Array

        let chunkSize = 10000;
        let arrayChunks = [];
        //array of Uint8SubArrays each of size chunk

        this.diffCount = 0;
        let progressCounter = 5000000;
        //we update progress every 5 million
        for (let i = 0; i < this.baseHardDrive.length; i++) {
            ...
            if (i > progressCounter) {
                ...
                this.rivetsData.message = "Diffs: " + this.diffCount + ", <b>" + percent + "%</b>";
        ...
        var xhr = new XMLHttpRequest;
        xhr.open("POST", this.rivetsData.settings.CLOUDSAVEURL + "/SendStaveState?name=" + this.base_name + '.doswasmx' + "&password=" + this.rivetsData.password + "&emulator=doswasmx", true);
        xhr.send(finalArray);
            ...
            if ((index + 1) < files.length) {
                ...
                if (myClass.rivetsData.noCopyImport) {
                    ...
                    for (let i = 0; i < myClass.importedFileNames.length; i++) {
                        ...
                        if (filename.toLocaleLowerCase().endsWith('.iso') || filename.toLocaleLowerCase().endsWith('.cue')) {
                            toastr.info('changing to ' + filename);
                            myClass.changeIso(filename);
                        }
                    }
                } else {
                    let importCommands = "mount e .\n" + "xcopy e:\\" + myClass.importFolderName + "\\*.* c:" + myClass.importFolderName + " /I /E\n" + "mount -u e\n" + "boot c:\n";
    ...
    processImportBuffer(name, buffer) {
        myClass.Run();
        myClass.rivetsData.showProgress = true;

        var byteArray = new Uint8Array(buffer);
        Module.FS.writeFile('/' + name, byteArray);

        myClass.rom_name = name;
        myClass.extractBaseName();

        if (name === "game.zip")
            myClass.configuration.startupScript = "go\r\n";
    ...
    loadCloud() {
        var oReq = new XMLHttpRequest();
        oReq.open("GET", this.rivetsData.settings.CLOUDSAVEURL + "/LoadStaveState?name=" + this.base_name + '.savestate.doswasmx' + "&password=" + this.rivetsData.password, true);
        oReq.responseType = "arraybuffer";
    ...
    async integrityCheck(newHardDriveBytes) {
        let finalByteArray = await this.loadHardDriveDiffs(this.baseHardDrive);
        //hard drive with applied diffs
        ...
        this.rivetsData.message = "Diffs: " + this.diffCount + " Final Array Size: " + finalsize.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " <b>DONE</b>";
